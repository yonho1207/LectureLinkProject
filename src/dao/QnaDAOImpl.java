...
public class QnaDAOImpl extends BaseDAO implements QnaDAO {
	...
	public List<Qna> selectLvlPage(int setRowStartNumber, int setRowEndNumber) {
		...
		try {
			connection = getConnection();
			preparedStatement = connection.prepareStatement(QnaSQL.QNA_SELECT_LVL_PAGE_SQL);
	...
	public Qna QnaComment(Qna qna) {
		...
		try {
			...
			connection = getConnection();
			preparedStatement = connection.prepareStatement(QnaSQL.QNA_CMT_INSERT_SQL);
	...
	/*
	 * @Override public int QnaCommentCnt(int grp) {
	 * 
	 * int cnt = 0; Connection connection = null; PreparedStatement
	 * preparedStatement = null; ResultSet resultSet = null;
	 * 
	 * try { connection = getConnection(); preparedStatement =
	 * connection.prepareStatement(QnaSQL.QNA_COMENT_CNT_SQL);
	 * preparedStatement.setInt(1, grp);
	 * 
	 * resultSet = preparedStatement.executeQuery();
	 * 
	 ...
	 * 
	 * closeDBObjects(resultSet, preparedStatement, connection); } return cnt;
	 * 
	 * }
	 */
}
